...
class ForeignKeySearchWindow(QMainWindow):
    ...
    class MainLayout(QVBoxLayout, QObject):
        ...
        class BottomLayout(QHBoxLayout, QObject):
            ...
            foreign_key_return_request = pyqtSignal(str)
            ...
            def button_pushed(self):
                print("Button pushed sending search request")
                sender_button = self.sender()
                index = self.table.indexAt(sender_button.pos())
                row_index = index.row()
                column_index = -1
                for i, column in self.main_layout.window.table.columns():
                    if column.name == "uuid":
                        column_index = i
                item = self.table.item(row_index, column_index)
                foreign_key_uuid = item.text()
                self.foreign_key_return_request.emit(foreign_key_uuid)
            ...
            def table_add_rows(self, rows):
                ...
                for i, row in enumerate(rows):
                    ...
                    for n, column in enumerate(self.main_layout.window.table.columns):
                        ...
                        button = QPushButton("Als Fremdschlüssel auswählen")
